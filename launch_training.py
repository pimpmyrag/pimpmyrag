...
# ── Config par défaut ─────────────────────────────────────────────────────────
DEFAULT_GOLD_VERSION = "v8.21_verbfam"  # ← seul endroit à mettre à jour entre versions
...
DEFAULT_GPU_PRIORITY = [
    ...
    # RTX 4090  : ~4-5 min/ep  $0.49/hr → ~$3.20 total  ← MEILLEUR RATIO
    # L40S      : ~3-4 min/ep  $0.75/hr → ~$3.75 total  ← FALLBACK PEU CHER
    # A6000     : ~3-4 min/ep  ~$0.80/hr  48GB
    # H100 SXM  : ~45-60s/ep  $3.49/hr → ~$4.20 total  ← CHER, dernier recours
    "NVIDIA GeForce RTX 4090",       # ~4-5 min/ep  $0.49/hr → ~$3.20 total
    "NVIDIA L40S",                   # ~3-4 min/ep  $0.75/hr → ~$3.75 total  (48GB, BS=96)
    "NVIDIA RTX A6000",              # ~3-4 min/ep  ~$0.80/hr (48GB, BS=96)
    "NVIDIA A40",                    # ~4-5 min/ep  ~$0.60/hr (48GB, BS=96)
    "NVIDIA A100-SXM4-40GB",         # ~2.5-3 min/ep $1.39/hr → ~$5.56 total (BS=96)
    "NVIDIA A100-SXM4-80GB",         # ~1.5-2 min/ep $1.99/hr → ~$5.00 total (BS=160)
    "NVIDIA A100 80GB PCIe",
    "NVIDIA H100 PCIe",              # ~1-1.5 min/ep $2.99/hr → ~$4.50 total
    "NVIDIA H100 80GB HBM3",         # ~45-60s/ep   $3.49/hr → ~$4.20 total  ← CHER
...
def build_start_cmd(ref: str, gold_version: str, loss_weighting: str = "fixed", config: str = None) -> str:
    ...
    return (
        ...
        f"cd pimpmyrag && git fetch origin && "
        f"(git reset --hard origin/{ref} 2>/dev/null || git reset --hard {ref}) && "
